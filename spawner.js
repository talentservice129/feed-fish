function Spawner(width, height, player, fishes) {
  ...
  this.zones = this.getAdjacentZones()
    .map(this.spawn.bind(this))
    .concat([this.currentZone]);
}
...
Spawner.prototype.update = function () {
  ...
  while (i--) {
    ...
    if (
      Math.abs(this.player.x - this.zones[i][0]) < this.width / 2 &&
      Math.abs(this.player.y - this.zones[i][1]) < this.height / 2
    ) {
    ...
    if (
      distance({ x: this.zones[i][0], y: this.zones[i][1] }, this.player) >
...
Spawner.prototype.getAdjacentZones = function () {
  ...
  var coords = dirs.map(function (dir) {
    return [
      dir[0] * self.width + self.currentZone[0],
      dir[1] * self.height + self.currentZone[1],
    ];
  });

  return coords;
};
Spawner.prototype.getRandomSize = function () {
